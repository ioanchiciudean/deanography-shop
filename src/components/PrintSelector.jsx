...
export default function ProductSelector({ imageUrl, title, slug, digitalPrice }) {
  ...
  const BOTH_DISCOUNT = 0.85; // 15% off when buying both
  const totalPrice = (() => {
  ...
  const [naturalRatio, setNaturalRatio] = useState(null); // null until image loads

  // Detect orientation from the actual image
  useEffect(() => {
    const img = new Image();
  ...
  // Preview dimensions — constrain to a fixed max box of 300x300
  const maxSize = 300;
  const ratio = naturalRatio ?? 3 / 4; // default to portrait while loading
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                {SIZES.map((s, i) => (
                  <button key={i} onClick={() => setSizeIdx(i)} style={{
                    background: "none",
                    border: `1px solid ${sizeIdx === i ? "#666" : "#222"}`,
              ...
              <div style={{ display: "flex", gap: 5 }}>
                {BORDERS.map((b, i) => (
                  <button key={i} onClick={() => setBorderIdx(i)} style={{
                    background: "none",
                    border: `1px solid ${borderIdx === i ? "#666" : "#222"}`,
                    color: borderIdx === i ? "#fff" : "#555",
                    padding: "8px 10px", cursor: "pointer", fontSize: 10, flex: 1,
                    transition: "all 0.15s",
                  }}>
                    {b.label}
                  </button>
